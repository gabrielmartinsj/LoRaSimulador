...
class myNode():
    ...
    def calculateADRatED(self):
        if self.packet.acked == 0:
            self.counter[1]+=1
        else:
            self.counter = [0,0]

        if self.counter == [0, 64] or self.counter == [1,32]:
            self.counter = [1, 0]
            if self.nexttxpow < 14:
                self.nexttxpow += 2
            elif self.nextsf < 12:
                self.nextsf += 1
    ...
    def newMargindB(self, DER_inst):
        ...
        if DER_inst < self.DER_ref:
            self.margin_db += 5
            if self.margin_db > 30:
                self.margin_db = 30
        elif DER_inst >= self.DER_ref*1.15 and self.margin_db > 10:
            self.margin_db -= 2.5
        else:
            self.margin_db = self.margin_db
        

    def calculateADRatNS(self):
        # Only keeps the highest RSSI of the same packet
        required_SNR = [-7.5, -10, -12.5, -15, -17.5, -20]

        if len(self.last_rssi_at_BS) !=0 and self.fcounter == self.last_rssi_at_BS[-1][1]:
            if self.last_rssi_at_BS[-1][0] < self.packet.rssi:
                self.last_rssi_at_BS[-1] = [self.packet.rssi, self.fcounter, self.packet.sf, self.packet.txpow]
        else:       
            self.last_rssi_at_BS.append([self.packet.rssi, self.fcounter, self.packet.sf, self.packet.txpow])
            if(self.buffer > datasize/2):
                self.recv += 1

        if len(self.last_rssi_at_BS) > 20:
            while len(self.last_rssi_at_BS) > 20:
                self.last_rssi_at_BS.pop(0)


            if ADR:
                SNR = [rssi[0]+174 - 10*math.log10(125e3) for rssi in self.last_rssi_at_BS]
                if ADRtype == "ADR-TTN":
                    SNRm = max(SNR)
                elif ADRtype == "ADR+":
                    SNRm = np.average(SNR)
                    ...
                    SNRm = np.average(SNR)
                    DER_inst = len(self.last_count)/float(self.last_count[-1]-self.last_count[0]+1)
                    self.newMargindB(DER_inst)

                self.snr.append(SNRm)
                # radio_sensitivity = required_SNR[node.packet.sf-7]#sensi[node.packet.sf - 7, 1]
                Nstep = int((SNRm - self.margin_db - required_SNR[node.packet.sf-7])/3)
            ...
            self.last_rssi_at_BS = [[self.packet.rssi, self.fcounter, self.packet.sf, self.packet.txpow]]
            if self.packet.acked == 1:
                self.nextsf = ADRsf
                self.nexttxpow = ADRtx
            else:
                self.nextsf = self.packet.sf
                self.txpow = self.packet.txpow
        else:
            self.nextsf = self.packet.sf
            self.txpow = self.packet.txpow
        ...
        if (node.packet.lost == False\
            ...
            # the ack is not lost
                ...
                if ADR:
                    ...
                    node.calculateADRatNS()
                    node.calculateADRatED()
            else:
            # ack is lost
